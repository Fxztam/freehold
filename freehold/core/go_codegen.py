...
class GoGenerator:
    ...
    def websocket_helpers(self) -> list[str]:
        self.std_imports.add("sync")
        self.std_imports.add("net")
        self.std_imports.add("bufio")
        self.std_imports.add("net/http")
        self.std_imports.add("net/url")
        self.std_imports.add("crypto/sha1")
        self.std_imports.add("encoding/base64")
        ...
        return [
            ...
            "}",
            "",
            "func wsReadLoop(conn net.Conn, inChan chan string) {",
            "\tdefer func() {",
            ...
            "\t\t}",
            "\t\tif opcode == 1 {",
            "\t\t\tinChan <- string(payload)",
            "\t\t}",
            "\t}",
            "}",
            "",
            "func wsWriteLoop(conn net.Conn, outChan chan string, mask bool) {",
            "\tdefer conn.Close()",
            "\tfor {",
            "\t\tmsg, ok := <-outChan",
            "\t\tif !ok {",
            "\t\t\t_ = writeFrame(conn, 8, nil, mask)",
            "\t\t\tbreak",
            "\t\t}",
            "\t\tif err := writeFrame(conn, 1, []byte(msg), mask); err != nil {",
            "\t\t\tbreak",
            "\t\t}",
            "\t}",
            "}",
    ...
    def routine_decl(self, routine: RoutineDecl) -> list[str]:
        ...
        if self.program.module_name == "WebSocket":
            if routine.name == "connect":
                self.std_imports.update({"context", "net", "net/url", "bufio", "net/http", "strings"})
                return [
                    ...
                    "\tgo wsReadLoop(conn, inChan)",
                    "\tgo wsWriteLoop(conn, outChan, true)",
                ...
                self.std_imports.update({"context", "net", "bufio", "net/http", "crypto/sha1", "encoding/base64"})
                return [
                    ...
                    "\toutChan := make(chan string, 64)",
                    "\tgo wsReadLoop(conn, inChan)",
                    "\tgo wsWriteLoop(conn, outChan, false)",
                    "\treturn ResultConnectionWebSocketError{",
                    "\t\tOk: true,",
                    "\t\tValue: Connection{",
