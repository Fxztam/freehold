...
DEFAULT_EXPECTED = Path("tests/language_modules_v2_3/expected_go_semantic_diagnostics.json")
DEFAULT_GO_ROOT = Path(".tmp/go-semantic-v2_3")


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify narrow Go-native semantic diagnostics for v2.3")
    ...
    for case, want in sorted(expected.items()):
        ...
        if not isinstance(diagnostics, list) or len(diagnostics) == 0:
            failures.append(f"{case}: expected semantic diagnostics, got {diagnostics!r}")
            continue

        # Find the diagnostic that matches the expected want
        matched = False
        for diagnostic in diagnostics:
            temp_failures = []
            check_diagnostic(temp_failures, case, diagnostic, want)
            if not temp_failures:
                matched = True
                break

        if not matched:
            failures.append(f"{case}: expected diagnostic {want!r} not found in {diagnostics!r}")

    if failures:
        print("Verify Go semantic diagnostics (v2.3)")
        print("-------------------------------------")
