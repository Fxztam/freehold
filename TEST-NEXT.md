...
- Aktueller Go-native Project-Semantic-Stand: `verify-go-semantic-projects.cmd` deckt 26 Faelle ab. Der letzte Slice ergaenzt negative Mehr-Package-Faelle fuer Result-`value.field`, Result-Array-`value[index].field`, importierte abortende Routine-Calls und `requires`-Ausdruecke mit importierten Routine-Calls. Der normale single-module Gate bleibt bei 18 Diagnostics.
- Naechster empfohlener Project-Semantic-Slice: strukturierte Diagnostics fuer Syntaxfehler in importierten Modulen, danach weitere negative Drei-Package-Goldens fuer hidden/non-exposed Symbole, falsche qualifizierte Modulnutzung und transitive Dependency-Fehler.
