...
- SymbolTable V0 sammelt Record-Typen, deren Felder und Function-Return-Typen aus dem Go-AST; `BuildSymbolTableWithImports` ergaenzt exposed importierte Records/Routinen inklusive transitiver Record-Feldtyp-Abhaengigkeiten.
- `ValidateModule` prueft lokale Routine-Parameter, `let`-Bindings und verschachtelte FieldAccess-Ausdruecke; `ValidateModuleWithImports` nutzt dieselben Regeln mit import-aware SymbolLookup. Die schmale Typinferenz deckt Literal-Basistypen, Routine-Return-Typen, einfache Operatoren und Array-Index-Elementtypen ab.
