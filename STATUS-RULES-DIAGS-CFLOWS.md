...
- `46/114` Diagnostic-Specs direkt im Go-Catalog.
- grob die `parse_syntax`-Schicht aus `freehold.rules` plus Go-native Record-/Array-/Routine-/Name-/Assignment- und Project-Loader-Semantikdiagnostics, also `45/115` Rules/Emits direkt im Go-Frontend.
...
- `go-semantic-project` laedt ein Entry-Modul samt Imports nach derselben Modulpfad-Konvention (`App.Main -> App/Main.fh`). `verify-go-semantic-projects.cmd` schreibt temporaere Artefakte nach `.tmp/go-semantic-project` und verankert 18 Projektfaelle fuer positive Imports, transitive Import-/Record-Kontexte und Project-Loader-Diagnostics. `verify-go-project-semantic-diagnostics.cmd` schreibt temporaere Artefakte nach `.tmp/go-project-semantic-diagnostics` und verankert 13 negative cross-module Semantikfaelle inklusive hidden/non-exposed Routine-Nutzung, falscher qualifizierter Modulnutzung, mehrfacher Diagnostics in einem Projekt sowie Result-/Array-/Abort-/Contract-Kombinationen. Mehrdeutig exposed Symbole werden Go-native mit `FH-SEM-1005 ambiguous_exposed_symbol` gemeldet; importierte Syntaxfehler werden als `FH-SEM-1011 imported_module_parse_error` golden-faehig.
...
- Go Diagnostic Catalog: `46/114` Diagnostic-Specs direkt in Go, davon 23 Syntax, 6 Typdiagnostics und 17 Semantikdiagnostics.
...
1. Project-aware Semantikdiagnostics weiter haerten, falls neue Cross-Module-Fehlerklassen auftauchen; strukturierte importierte Parse-Fehler und die wichtigsten negativen Drei-Package-Goldens sind verankert.
2. Den separaten `verify-go-project-semantic-diagnostics.cmd`-Gate fuer neue negative project-aware Semantikgoldens beibehalten; der bestehende Project-Gate bleibt fuer Loader-/OK-Projekte fokussiert.
